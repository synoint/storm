...
if (!window.synoES) {
    ...
    link.type = "text/css";
    link.rel = "stylesheet";
    link.href = 'https://survey.synointcdn.com/embed_survey/v1/s-survey.css';
	...
	synoES.util = synoES.util || {};
	synoES.util.survey = synoES.util.survey || {};

	var synoES_SETTINGS = {
        CONTAINER_ID: "",
        SESSSION_DURATION: 55,//seconds
	...
	function InvitationTimeoutHandler(settings) {
        ...
        function run_(settings) {
            ...
            if((now - lastSessionActivity)/1000 > synoES_SETTINGS.SESSSION_DURATION){
                sessionStart = now;
                synoES_Cookie.set(synoES_SETTINGS.SESSION_START_COOKIE, sessionStart, 1);
            }
            ...
            if(sessionDuration > synoES_SETTINGS.INVITATION_TIMEOUT){
                synoES.util.survey.show(settings);
                return;
            }
    ...
    synoES.init = function(settings) {
        ...
        settings.titleText = settings.titleText || '👋 Hej, vill du vara med och utveckla Filmstaden webbplats?';
        settings.hintText = settings.hintText || 'Tryck "Nästa" nedan! Ditt svar är anonymt.';
        settings.buttonLabel = settings.buttonLabel || 'Nästa';
    ...
    synoES.util.survey.show = function(settings){
        var rootElement = document.getElementById(synoES_SETTINGS.CONTAINER_ID);
        rootElement.innerHTML = synoES.util.survey.getSurveyTemplate(settings);
    }

    synoES.util.survey.closeInvitationPopup = function(){
        var rootElement = document.getElementById(synoES_SETTINGS.CONTAINER_ID);
        rootElement.parentNode.removeChild(rootElement);
    };

    synoES.util.survey.updateInvitationPopupVisibility = function(){
        var containerStyle = document.querySelector('.sss-invitation-container').style;
        var expandCollpseButtonStyle = document.querySelector('.sss-collapse-button-icon').style;
        if(containerStyle.transform){
            containerStyle.removeProperty('transform');
            expandCollpseButtonStyle.backgroundPosition = '-32px 0';
            ...
            expandCollpseButtonStyle.backgroundPosition = '0 0';
        }
    };

    synoES.util.survey.loadSurvey = function(synoSurveyURL){
    ...
    synoES.util.survey.getSurveyTemplate = function(settings){
        var tempalteChunks = [
            ...
            'onclick="return synoES.util.survey.updateInvitationPopupVisibility();"><span class="sss-collapse-button-icon"></span></button>',
            '<button class="sss-close-button" style="background: ' + settings.containerColor + ';" ',
            'onclick="return synoES.util.survey.closeInvitationPopup();"><span class="sss-close-button-icon">x</span></button>',
            ...
            '<button type="button" onclick="synoES.util.survey.loadSurvey(\''+settings.surveyURL+'\');" style="background-color: ' + settings.buttonColor + ' !important;" class="sss-action-button">',
